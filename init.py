...
def getsurecommands(tokens, commandslist):
    ...
    flag = True
    for x in tokens:
        if x.type == 'IDENTIFIER' and (out.get(x.value) is not None) and flag:
            out[x.value] += 1
            flag = False
        elif x.type == 'DF' or x.type == 'OR' or x.type == 'BAND':
            flag = True
    return out


def getsystemcommand(tokens, commandslist):
    tem = commandslist
    stemcommandlist = []
    temstr = ''
    state = 0
    flag = True
    for x in tokens:
        if x.type == 'IDENTIFIER' and (tem.get(x.value) is not None) and flag and state == 0:
            if x.value == 'systemctl':
                state = 1
            flag = False
        elif state == 1:
            temstr += 'systemctl ' + x.value
            state = 2
        elif state == 2:
            temstr += ' ' + x.value
            state = 0
            stemcommandlist.append(temstr)
            temstr = ''
        elif x.type == 'DF' or x.type == 'OR' or x.type == 'BAND':
            flag = True
...
def init(data):
    ...
    while True:
        ...
        tokens.append(tok)
    # 获取带有重启关闭开启服务的命令
    lll = getsystemcommand(tokens, getcommandslist())
    # 获取可能调用的命令
